...
async Dashboardtab(module: string) {
    await new dashboardModules(this.page).clickModuleTab(module);
}
async target(){
...
async targetvalue(value:string){
 ...
 async addcondition(moduleLabel = 'Leads'){
    ...
    const option=this.page.getByLabel(moduleLabel).getByRole('treeitem', { name: 'Created By', exact: true });
    await option.click();
}
...
async clickTarget(){
  ...
  /** True only when Targets tab and the add (+) control are visible on the current module. */
  async hasTargetPermission(): Promise<boolean> {
    const targetTab = this.page.locator('[id="baseIcon-tab3"]');
    const tabVisible = await targetTab.isVisible({ timeout: 0 }).catch(() => false);
    if (!tabVisible) {
      return false;
    }

    await targetTab.click().catch(() => {});
    await this.page
      .locator('#livewireOverly')
      .waitFor({ state: 'hidden', timeout: 15_000 })
      .catch(() => {});

    const targetAddButton = this.page.getByText('add', { exact: true }).nth(1);
    return targetAddButton.isVisible({ timeout: 3_000 }).catch(() => false);
  }

  /** Dismiss customizers, filter/target/widget wizards, and overlays blocking dashboard tab clicks. */
  async dismissCustomizerOverlays() {
    ...
    const closeSelectors = [
      ...
      this.page.locator('[class="dashFilterclrBtn"]').first(),
