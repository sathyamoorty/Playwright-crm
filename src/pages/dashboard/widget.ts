...
async clickMod(index: number) {
   ...
   async openCreateWidget(indexOrModule: number | string) {
       await this.dashIcon();
       if (typeof indexOrModule === 'number') {
           await this.clickMod(indexOrModule);
       } else {
           await new dashboardModules(this.page).clickModuleTab(indexOrModule);
       }
       await this.clickWid();
       await this.clickAdd();
       await expect(this.page.locator('#widgetname')).toBeVisible({ timeout: 30000 });
   ...
   /** True only when Widgets tab and #createWidget add button are visible on the current module. */
   async hasWidgetPermission(): Promise<boolean> {
       const widgetsLink = this.page.getByRole('link', { name: 'widgets', exact: true });
       const linkVisible = await widgetsLink.isVisible({ timeout: 0 }).catch(() => false);
       if (!linkVisible) {
           return false;
       }

       await widgetsLink.click().catch(() => {});
       await this.page
           .locator('#livewireOverly')
           .waitFor({ state: 'hidden', timeout: 15_000 })
           .catch(() => {});

       const widgetAddButton = this.page.locator('#createWidget');
       return widgetAddButton.isVisible({ timeout: 3_000 }).catch(() => false);
   }
